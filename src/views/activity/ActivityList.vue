...
async function handleDelete(activity: Activity): Promise<void> {
  try {
    await ElMessageBox.confirm(
      `确定要删除活动"${activity.name}"吗？删除后不可恢复。`,
      '删除确认',
      {
...
async function handleArchive(activity: Activity): Promise<void> {
  try {
    await ElMessageBox.confirm(
      `确定要归档活动"${activity.name}"吗？归档后活动将不可编辑。`,
      '归档确认',
      {
    ...
    <div class="page-container">
      ...
      <div class="content-card" v-loading="loading">
        <div class="filter-bar">
          ...
          <!-- 筛选结果提示 -->
          <div
            v-if="appliedType || appliedStatus || appliedTimeRange || appliedKeyword"
            class="filter-result-tip"
          >
            当前筛选条件：
            ...
            <el-tag
              v-if="appliedTimeRange"
              size="small"
              closable
              @close="appliedTimeRange = null"
            >
        ...
        >
          ...
          <el-table-column label="活动类型" width="110" align="center">
            <template #default="{ row }">
              <el-tag
                :color="activityTypeColorMap[row.type]"
                effect="light"
                size="small"
              >
                {{ row.type }}
          ...
          <el-table-column label="活动状态" width="100" align="center">
            <template #default="{ row }">
              <el-tag
                :color="activityStatusColorMap[row.status]"
                effect="light"
                size="small"
              >
                {{ row.status }}
